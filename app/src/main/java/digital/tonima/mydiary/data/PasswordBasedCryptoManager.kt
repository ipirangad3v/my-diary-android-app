...
object PasswordBasedCryptoManager {
    ...
    fun saveDiaryEntry(context: Context, filename: String, content: String, masterPassword: CharArray) {
        try {
            // 1. Generate a new random salt for each entry.
            ...
            val iv = cipher.iv // The IV is generated by the cipher in GCM mode.
            val encryptedContent = cipher.doFinal(content.toByteArray())

            // 4. Write the salt, IV, and encrypted data to the file.
    ...
    fun readDiaryEntry(context: Context, filename: String, masterPassword: CharArray): String? {
        return try {
            ...
            FileInputStream(file).use { fis ->
                // 1. Read the salt and IV from the beginning of the file.
                val salt = ByteArray(SALT_SIZE)
                fis.read(salt)
                ...
                // 2. Read the rest of the file (the encrypted content).
                val encryptedContent = fis.readBytes()

                // 3. Derive the same key using the password and the read salt.
                val key = deriveKey(masterPassword, salt)
                val cipher = Cipher.getInstance(TRANSFORMATION)
                val spec = GCMParameterSpec(128, iv) // 128-bit authentication tag for GCM.
